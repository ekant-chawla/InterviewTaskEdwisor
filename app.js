...
//Cron for deletion of expired tokens
const Auth = mongoose.model('Auth')

setInterval(() => {
    console.log('deleting expired tokens')

    let expiryTimeInMS = config.tokenExpiry * 60 * 60 * 1000
    let createdOnBoundry = Date.now() - expiryTimeInMS

    Auth.deleteMany({ createdOn: { $lte: createdOnBoundry } }).exec((err, result) => {
        if (err) {
            logger.error(`Failed to delete authtoken before ${createdOnBoundry}`, 'Auth token cron', 5)
        }
    })
}, config.tokenExpiry * 60 * 60 * 1000)